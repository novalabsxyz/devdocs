...
Go into rebar.config and change the blockchain-core over from git to http like so:
...
Next we will need to change the repo we are using for rocksdb in rebar.lock like so:
...
Next, we'll need to install the following depedencies before we can compile concentrate:
