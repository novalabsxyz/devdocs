...
There are two distinct applications required in order for the Hotspot to send and receive LongFi packets, and route them to the correct internet hosted destination. [miner](https://github.com/helium/miner) connects to the blockchain and forwards received packets to the correct address by inspecting an Organizationally Unique Identifier in the blockchain ledger, and [concentrate](https://github.com/helium/concentrate) interfaces with the LoRa concentrator via SPI and the miner via UDP to send and receive LongFi packets.
...
First, [rustup](https://rustup.rs) must be installed, which can be done as follows:
...
Please check out the [Helium for Developers](https://helium.com/developers) site for more information on how the rest of the system works.