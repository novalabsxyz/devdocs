...
[Acoounts, Wallets, and Keys](#accounts-wallets-and-keys)
...
The easiest way to see the blocks being mined is with the [Block Explorer](https://network.helium.com/blocks). The Helium Mobile App also shows block details. Developers can also use the Helium blockchain API to access all block transaction and metadata.
...
The Helium blockchain currently has 20 native transaction types. Some appear in nearly every block mined while others were created to be used only once and will never heard from again. (You can see various transaction types in action using the Block Explorer](https://network.helium.com/blocks). 
...
[accounts-wallets-keys]: #accounts-wallets-and-keys
