...
Below are the mining rewards per epoch. That is to say, for every complete epoch (marked by the election of a new Consensus Group), all the `HNT` produced get distributed over the following reward types:
...
The last thing to examine is the amout of `HNT` earned per Hotspot in a given epoch, taking into acount the total number of Hotspots live on the Network eligible for each category of `HNT` reward. 

** A Few Notes on Reward Types:**

* All the Hotspots that have earned a specific reward type will split it equally. 
...
--- TODO --- Find a block with 


| Reward Type                 | Hotspots Earning Reward    			| HNT Earned Per Hotspot |
| -----------   	          | ----------- | -----------           |
| PoC Challenger              |             | 513.69                |
| PoC Challengees             |             | 1198.63               |
| Witnesses                   |             | 171.24       			|
| Consensus Group   		  | 16          | 22.9       			|
| Helium, Inc and Investors   | N/A         | 1198.63      			|
| Network Data Transfer       | 0%          | 0            			|
| **Total**                   | **100%**    | **3424.66**  			|
