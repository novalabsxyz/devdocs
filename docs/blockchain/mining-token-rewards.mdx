...
Every [epoch](#), the current consensus group mines a fixed amount of `HNT` which then gets distributed to all Hotspots that have earned it. Hotspots and other network participants are rewarded across the following reward types:
