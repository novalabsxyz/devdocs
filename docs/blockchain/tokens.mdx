...
* The Helium Token, used to reward Hotspot Operators and Helium Network Investors 
* Data Credit, derived from HNT in an on-chain burn transaction and used to pay transaction fees on the Helium Network
...
The first HNT was produced on July 29th, 2019 at 09:43PM UTC as part of the genesis block that was mined with the Helium Network launch. Prior to this date, there was no premine of HNT. The target mining rate is 5,000,000 HNT per month (as defined in the “monthly_reward” chain variable). 5,000,000 HNT are produced where the target blocktime and epoch times (as defined in their respective chain variables) are achieved over a one month period.

One HNT is made up of 10,000,000 “bones”. Therefore, the smallest piece of a HNT that can be used as part of a transaction is 0.00000001, or 1 bone.
...
A more complete breakdown of the token reward distribution can be found here.

### Data Credits

(NOTE: Data Credits are not yet implemented. We expect this to be live in Q1 2020.)

Data Credits (DCs) are used to pay all transaction fees on the Helium Network. DCs are created using an on-chain burn transaction which converts HNT to DCs at a rate pegged to the $US. In other words, the price of a DC will always be fixed. However, the amount of DCs produced by burning HNT will move up and down based on the $USD price of HNT. 
