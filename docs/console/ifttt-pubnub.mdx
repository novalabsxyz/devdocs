...
At this point you could pick any one of thousands of actions like sending a tweet, text, or even turning on a smart light bulb, but for simplicity use the email service to send a simple email with the payload each time a packet is forwarded.
