...
If you are not within range, you can [build your own Hotspot](/hotspot/developer-setup) or purchase one from the <a href="https://helium.com/store">Helium Store</a>. There is special pricing for Developers, so please reach out to developer@helium.com before making a purchase.
...
Helium Console enables developers to add devices and grab the device identifiers to load onto their device firmware that authenticates and identifies the device for the Helium Network.
