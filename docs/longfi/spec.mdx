...
import protocol from '../images/longfi/protocol.png' 
import example_payload from '../images/console/requestbin_example.png'

# Helium LongFi Protocol

This specification introduces the high-level semantics of LongFi, the Helium network's wireless protocol.
...
[LoRa Physical Layer](#lora-physical-layer)
...
- Downlink: 64 downlink channels utilizing LoRa 125 kHz bandwidth using coding rate 4/5. The channels are numbered 1 to 64 and the frequencies are to be determined.
...
{
    "device_id": 36
    "fingerprint": 3714365718
    "gateway": "faithful raisin wolverine"
    "oui": 1
    "payload": "eyJ2YWx1ZTEiOiJ0ZXN0MSIsInZhbHVlMiI6InRlc3QyIiwidmFsdWUzIjoidGVzdDMifQ=="
    "rssi": -68
    "sequence": 2
    "snr": 11.75
    "spreading": "SF10"
    "timestamp": 1575574722
}
```

The payload, as it appears as a HTTP POST on <a href="https://www.requestbin.com">Requestbin </a>. 

<img src={example_payload} />

The decoded user **payload** for this example is:
...
* **gateway** : name of the Hotspot that received the transmitted LongFi packet. 
...
* **rssi** : Received Signal Strength Indicator, an estimate measure of power level that the client device is receiving from a Helium Hotspot. 
* **sequence** : Ordering of byte stream. 
