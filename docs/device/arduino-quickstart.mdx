...
> ##### Important!
>Before we begin, please make sure you've followed the steps from this [guide](/console/setup-device), which goes over some initial setup steps. 
...
That’s it for the hardware setup! Next we will setup your environment. We are going to use the Arduino IDE here. Don't have to. We're going to. Will update with instructions for <a href="https://platformio.org/" target="_blank">PlatformIO</a> later on, which is what you really should be using anyways...
...
The MCCI LoRaWAN LMIC Library hmay be installed from the Arduino IDE 
...
Create a new sketch and copy the source code over. All you need to do is update the DevEUI, AppEUI, and AppKey. DevEUI (LSB), AppEUI (LSB), and AppKey (MSB) are provided from Helium Console when you created your device:
...
Be sure to select the appropriate format (LSB/Little-Endian for DevEUI/AppEUI and MSB/Big-Endian for AppKey) and copy the information into the sketch:
