...
Not that the byte ordering of the `DevEUI`, `AppEUI`, and `AppKey` are important. 

* `DevEUI` and `AppEUI`(MSB) need to be copied over in the `LSB` format. 
