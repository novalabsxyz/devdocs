...
If you do not see any hotspots in your area, you would need to purchase one from the <a ref="https://helium.com/store" target="_blank">Helium Store</a>. There is special pricing available for Developers, so please reach out to developer@helium.com before making a purchase. 
...
For the initial Beta, we only support one OUI - Organizationally Unique Identifier for routing device packets. In the TransmitPacket sketch, make sure the OUI is set properly. The device_id should correspond with the device you onboard on Console.
...
To see what other developers are working on, join the Helium Community <a href="https://community.helium.com/
