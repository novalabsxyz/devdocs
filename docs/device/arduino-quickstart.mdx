...
> ##### Important! 
> If you have already installed the LongFi library, please make sure to check for updates
...
There are two example sketches included with the LongFi library. TransmitPacket will send a stream of packets while ButtonTransmit will transmit packets on-demand upon button press. 

To learn more about the LongFi packet payload, check out this [section] (/longfi/spec#payload).

Arduino IDE:

```
File -> Examples -> LongFi -> TransmitPacket
File -> Examples -> LongFi -> ButtonTransmit
```
